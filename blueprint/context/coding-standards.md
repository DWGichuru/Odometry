...
This project uses the App Router at the repo root (no `src/` directory).
...
> Only `app/` exists so far; create the sibling folders above at the repo root as
> the code needs them.
...
- Tailwind CSS for all styling (Tailwind v4 is installed via `@tailwindcss/postcss`)
- Tailwind v4: CSS-first config (`@theme` in `app/globals.css`), no `tailwind.config.js`
- No inline styles
- Dark mode first, light mode as option

> TODO: no component library is installed yet. If you add one (shadcn/ui or
> similar), record the convention here.

## Database

> TODO: no database or ORM is installed yet. The defaults below assume Prisma;
> confirm the choice when data storage is added, then keep or replace them.
...
> TODO: no auth or validation library is installed yet. The defaults below assume
> Clerk for auth and Zod for validation; confirm when those are added.

- Server components fetch directly with Prisma
- Client components use Server Actions
- Validate all inputs with Zod
- Scope every user-owned query by the authenticated user id; never trust a client-supplied user id
